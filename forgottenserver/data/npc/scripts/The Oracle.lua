...
local config = {
	...
	vocations = {
		["mage"] = {
			text = "A MAGE! ARE YOU SURE? THIS DECISION IS IRREVERSIBLE!",
			vocationId = 1,
		...
		["cleric"] = {
			text = "A CLERIC! ARE YOU SURE? THIS DECISION IS IRREVERSIBLE!",
			vocationId = 2,
		...
		["scout"] = {
			text = "A SCOUT! ARE YOU SURE? THIS DECISION IS IRREVERSIBLE!",
			vocationId = 3,
		...
		["fighter"] = {
			text = "A FIGHTER! ARE YOU SURE? THIS DECISION IS IRREVERSIBLE!",
			vocationId = 4,
...
local function creatureSayCallback(cid, type, msg)
	...
	if npcHandler.topic[cid] == 0 then
		if msgcontains(msg, "yes") then
			npcHandler:say("IN WHICH CITY DO YOU WANT TO LIVE? {KARMIA} IS THE ONLY CHOICE.", cid)
	...
	elseif npcHandler.topic[cid] == 1 then
		...
		if cityTable then
			town[cid] = cityTable
			npcHandler:say("IN ".. string.upper(msg) .."! AND WHAT PROFESSION HAVE YOU CHOSEN: {FIGHTER}, {SCOUT}, {MAGE}, OR {CLERIC}?", cid)
			npcHandler.topic[cid] = 2
		else
			npcHandler:say("IN WHICH TOWN DO YOU WANT TO LIVE? {KARMIA} IS THE ONLY CHOICE.", cid)
		end
	elseif npcHandler.topic[cid] == 2 then
		...
		if vocationTable then
			...
			npcHandler:say("{FIGHTER}, {SCOUT}, {MAGE}, OR {CLERIC}?", cid)
		end
	elseif npcHandler.topic[cid] == 3 then
		if msgcontains(msg, "yes") then
			...
			npcHandler:say("THEN WHAT? {FIGHTER}, {SCOUT}, {MAGE}, OR {CLERIC}?", cid)
			npcHandler.topic[cid] = 2
		end
	end
	return true
