			...
			for i = 1, #started do
				...
				if player:getStorageValue(KILLSSTORAGE_BASE + id) >= tasks[id].killsRequired then
					for j = 1, #tasks[id].rewards do
						...
						elseif isInArray({REWARD_COINS, 'coins', 'coin', 'tibiacoins'}, reward.type:lower()) and not deny then
							doPlayerAddCoins(cid, reward.value[1])
    						doPlayerSendTextMessage(cid, MESSAGE_EVENT_ADVANCE, "You received "..reward.value[1].." Tibia Coins.")
    					end
