...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 1 then	
			npcHandler:say({"We have introduced a bonus system. Each time you deposit 100 vials without claiming the money for it, you will receive a lottery ticket. ...",
		...
		elseif npcHandler.topic[cid] == 3 then	
			if player:removeItem(7634, 40) or player:removeItem(7635, 20) or player:removeItem(7636, 100) then
				npcHandler:say("Ok here take this Lottery Ticket!", cid)
				player:addItem(5957,1)
				npcHandler.topic[cid] = 0
			else
				npcHandler:say("You dont have 100 small vials, 40 strong vials or 20 great vials.", cid)
				npcHandler.topic[cid] = 0
			end
		elseif npcHandler.topic[cid] == 4 then	
			...
			elseif player:getItemCount(5958) >= 1 then
				...
				npcHandler:say("Congratulations! Here, you won 30000 Gold!", cid)
				npcHandler.topic[cid] = 0
			else
