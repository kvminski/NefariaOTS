...
local function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, "mission") or msgcontains(msg, "report")) then
		if(player:getStorageValue(Storage.TheInquisition.Questline) == 1) then
			npcHandler:say({"Let's see if you are worthy. Take an inquisitor's field guide from the box in the back room. ...","Follow the instructions in the guide to talk to the City guards that protect the walls and gates of the city and test their loyalty. Then report to me about your {mission}."}, cid, 0, 1, 3000)
			npcHandler.topic[cid] = 0
		...
		elseif(player:getStorageValue(Storage.TheInquisition.Questline) == 11) then
			npcHandler:say({"We've got a report about an abandoned and haunted dirty house on the swamps near the City. I want you to examine this house. There is old abandoned warehouse near the ruins so you should have no trouble finding it. ...","There's an evil being somewhere. I assume that it will be easier to find the right spot at night. Use this vial of holy water on that spot to drive out the evil being."}, cid, 0, 1, 3500)
		...
		elseif(player:getStorageValue(Storage.TheInquisition.Questline) == 12 or player:getStorageValue(Storage.TheInquisition.Questline) == 13) then
			npcHandler:say("Your current mission is to exorcise an evil being from a ruins on the swamps near the City. Are you done with that mission? ", cid)
		...
		elseif(player:getStorageValue(Storage.TheInquisition.Questline) == 17) then
			npcHandler:say({"We've got information about something very dangerous going on in old mage academy on the swamps. The demons are preparing something there ...","Something that is a threat to all of us. Our investigators were able to acquire vital information before some of them were slain by a demon named Ungreez. ...","It'll be your task to take revenge and to kill that demon. You'll find him in one of the towers on the Academy. Good luck."}, cid, 0, 1, 3200)
			npcHandler.topic[cid] = 0
	...
	elseif msgcontains(msg, 'king') then
		npcHandler:say('The Neptune king is crowned by a representative of the churches. This means they reign in the name of the gods of good and are part of the godly plan for humanity. ...', cid)
		...
		npcHandler:say('The king fund the inquisition and sometimes provide manpower in matters of utmost importance. The inquisition, in return, protects the realm from heretics and individuals that aim to undermine the holy reign of the kings.', cid, 7000)
		npcHandler.topic[cid] = 0
	...
	elseif msgcontains(msg, 'fafnar') then
		...
		npcHandler.topic[cid] = 0
	elseif msgcontains(msg, 'ithilien') then
		npcHandler:say('Ithilien is a city of sin and heresy. After the reunion of Ithilien with the kingdom, the inquisition will have much work to purify the city and its inhabitants.', cid)
		npcHandler.topic[cid] = 0
	elseif msgcontains(msg, 'demon') then
		...
		npcHandler.topic[cid] = 0
	elseif msgcontains(msg, 'azshara') then
		npcHandler:say('Azshara is a godless town full of mislead fools. One day, it will surely share the fate of its sister town Drefia.', cid)
		npcHandler.topic[cid] = 0
	elseif msgcontains(msg, 'drefia') then
		...
		npcHandler.topic[cid] = 0
	elseif msgcontains(msg, 'slugmyn') then
		npcHandler:say('Slugmyn is somewhat difficult to handle. The merchants have a close eye on our activities in their city and our authority is limited there. However, we will use all of our influence to prevent a second Ithilien.', cid)
		npcHandler.topic[cid] = 0
	elseif msgcontains(msg, 'capitlif') then
		npcHandler:say('Capitlif illustrates perfectly why the inquisition is needed and why we need more funds and manpower. ...', cid)
		...
		npcHandler.topic[cid] = 0
	elseif msgcontains(msg, 'ancientic') then
	...
	elseif msgcontains(msg, 'druid') then
		npcHandler:say('The druids here still follow the old rules. Sadly, the druids of Ithilien have left the right path in the last years.', cid)
		npcHandler.topic[cid] = 0
